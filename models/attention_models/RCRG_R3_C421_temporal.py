...
class GroupActivityClassifer(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.r1(x[:,  :3, :], edge_index)       # (b*seq, bb/4, 128) {0, 1, 2}                                     
        x2 = self.r1(x[:, 3:6, :], edge_index)       # (b*seq, bb/4, 128) {3, 4, 5} 
        x3 = self.r1(x[:, 6:9, :], edge_index)       # (b*seq, bb/4, 128) {6, 7, 8}
        x4 = self.r1(x[:, 9: , :], edge_index)       # (b*seq, bb/4, 128) {9, 10, 11}                                                     
        x_r1 = torch.concat([x1, x2, x3, x4], dim=1) # (b*seq, bb,   128)     
        ...
        x1 = self.r2(x[:, :6, :], edge_index) # (b*seq, bb/2, 256) {0, 1, 2, 3, 4, 5}                                  
        x2 = self.r2(x[:, 6:, :], edge_index) # (b*seq, bb/2, 256) {6, 7, 8, 9, 10, 11}                                                    
        x_r2 = torch.concat([x1, x2], dim=1)  # (b*seq, bb,   256)     
        
        # The thrid relational layer has 1 cliques
        num_nodes = x.shape[1]  # all 12 player at one graph 
        edge_index = torch.tensor([(i, j) for i, j in itertools.permutations(range(num_nodes), 2)]).t().to(self.device) # Generate all (i, j) pairs where i ≠ j
        x_r3 = self.r3(x, edge_index) # (b*seq, bb, 512)

        x = torch.concat([x_r1, x_r2, x_r3], dim=2) # (b*seq, bb, 896)
        team_1 = self.pool(x[:, :6, :]) # (b*seq, 1, 896) 
        team_2 = self.pool(x[:, 6:, :]) # (b*seq, 1, 896) 
        
        x = torch.concat([team_1, team_2], dim=1) # (b*seq, 2,  896) 
        x = x.view(b*bb, seq, -1)                 # (b, seq, 1792)

        x, (h, c) = self.lstm(x)                          # (b, seq, 1024)
        x = x[:, -1, :]                                   # (b, 1024)
