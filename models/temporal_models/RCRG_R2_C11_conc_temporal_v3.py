"""
RCRG-R2-C11-conc-temporal-v3 (RCRG-2R-11C-conc) Description:
--------------------------------
...
class GroupActivityClassifer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.contiguous().view(b*bb, seq, -1)         # (b*bb, seq, 384)
        x, (h, c) = self.lstm2(x)         # (b*bb, seq, 384)
        x = x[:, -1, :]                   # (b*bb, 384)
...
def eval(root, config, checkpoint_path):
    ...
    prefix = "Group Activity RCRG_R2_C11-conc-V3 eval on testset"
